...
function fwbc_process_page(&$vars){
	...
	if (!strlen($page_alias)) { $page_alias = $alias[count($alias) - 2]; }
	...
	if (count($menu_tree)) {
		...
		foreach($menu_tree as $menu_item) {
			if($menu_item['link']['hidden']==0){
				...
				$ul_li_class = '';
				$menu_nmb--;
				if ($menu_nmb == 1) { $ul_li_class = ' class="last"'; }
			}
		}
		$menu_html .= '</ul>'.chr(10);
	}
	...
	$menu_tree = menu_tree_all_data($menu_delta);
	 if (count($menu_tree)) {
		...
		foreach($menu_tree as $menu_item) {
			...
			$menu_nmb--;
			if ($menu_nmb == 1) { $ul_li_class = ' class="last"'; }
			if (strlen($lisep)) { $menu_lisep = $lisep; }
		}
...
if (strlen(@$_POST['form_id'])) {
	if ($_POST['form_id'] == 'webform_client_form_'.theme_get_setting('fwbc_newsletter_signup_form')) {
		$cmname = trim($_POST["submitted"]["name"]);
		$cmemail = trim($_POST["submitted"]["email"]);
		if (strlen($cmname) && preg_match("/^[a-zA-Z0-9\.\_\-]+@[a-zA-Z0-9\-]+\.[a-zA-Z0-9\-\.]+$/", $cmemail)) {
			$cmurl = theme_get_setting('fwbc_newsletter_cm_subscribe_url');
			if (strlen($cmurl)) {
				if (substr($cmurl, -1) != '/') { $cmurl .= '/'; }
				$cmurl_array = explode('/', $cmurl);
				$emnm = $cmurl_array[count($cmurl_array) - 2].'-'.$cmurl_array[count($cmurl_array) - 2];
				$postfields = array('cm-name' => $cmname, 'cm-'.$emnm => $cmemail);
				$ch = curl_init($cmurl);
